...
public class ThirdFigure extends DrawingTool {
    ...
    public ThirdFigure(Bitmap mainBitmap, Bitmap fakeBitmap) {
    ...
    private void drawFigure(int x1, int y1, int x2, int y2, Bitmap bitmap, RenderingType renderingType) {
        ...
        int th = Math.round(b / 8);
        int tw = Math.round(a / 8);

        points[0] = new Vertex(x1, y1 + 4 * th * directionY);
        points[1] = new Vertex(x1 + 2 * tw * directionX, y1);
        points[2] = new Vertex(x1 + 2 * tw * directionX, y1 + 2 * th * directionY);
        points[3] = new Vertex(x1 + 6 * tw * directionX, y1 + 2*th * directionY);
        points[4] = new Vertex(x1 + 6*tw * directionX, y1);
        points[5] = new Vertex(x1 + 8 * tw * directionX, y1 + 4*th*directionY);
        points[6] = new Vertex(x1 + 6 * tw * directionX, y1 + 8*th * directionY);
        points[7] = new Vertex(x1 + 6 * tw * directionX, y1 + 6*th * directionY);
        points[8] = new Vertex(x1 + 2 * tw * directionX, y1 + 6 * th * directionY);
        points[9] = new Vertex(x1 + 2 * tw * directionX, y1 + 8 * th * directionY);


        for (int i = 0; i < 10; i++) {
            painter.drawDDALine(points[i].getX(), points[i].getY(),
                    points[(i + 1) % 10].getX(), points[(i + 1) % 10].getY(),
