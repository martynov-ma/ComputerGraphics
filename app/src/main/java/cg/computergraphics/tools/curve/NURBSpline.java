...
public class NURBSpline extends DrawingTool {
    ...
    public NURBSpline(Bitmap mainBitmap, Bitmap fakeBitmap) {
    ...
    private void drawNURBSpline(ArrayList<Point> points, Bitmap bitmap) {
        ...
        while (u <= temp) {
            ...
            resYZn = 0;
            for (int i = 0; i < n; i++) {
                float f = N(i, rank, u, t);
                resXCh += points.get(i).x * f * weights[i];
                resYCh += points.get(i).y * f * weights[i];
            ...
            try {
                bitmap.setPixel((int) (resXCh / resXZn), (int) (resYCh / resYZn), super.getColor());
        ...
        switch (motionEvent.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (points.size() < rank) {
                    ...
                    if (points.size() == rank) {
                        drawNURBSpline(points, super.getFakeBitmap());
                    }
                } else {
                    check(x, y);
                    if (!isEditingPoint) {
